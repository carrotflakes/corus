...
use corus::{
    contrib::{
        ...
        perlin_noise, resetable_acc,
        simple_comp::SimpleComp,
    },
    core::{
        add::Add,
        amp::Amp,
...
fn main() {
    let file = std::env::args()
        .skip(1)
        .next()
        .unwrap_or("./yorunikakeru.mid".to_string());
    ...
    let node = Amp::new(mix, Var::new(0.25));
    // let node = corus::contrib::down_sample::DownSample::new(node, C2f64([0.0, 0.0]), 4410);
    let node = delay_fx(node, SAMPLE_RATE as usize, 0.3, 0.3);
    let node = SimpleComp::new(
        node,
        Var::from(0.5),
        Var::from(0.5),
        Var::from(1.5),
    ...
    write_to_file::write_to_file(file.as_str(), SAMPLE_RATE, time, node, Some(0x17b7f5f0e4a237bf), Some(0x9e43acd7755f2d74));
    println!("saved {:?}", &file);
}
...
fn saw_builder(pitch: Share<Controllable<Param<f64>>>) -> MyVoice {
    ...
    Voice(
        Box::new(node) as Box<dyn Node<Output = C1f64> + Send + Sync>,
...
fn benihora_builder() -> MyVoice {
    ...
    Voice(
        Box::new(Amp::new(benihora, Var::from(1.5))) as Box<dyn Node<Output = C1f64> + Send + Sync>,
        Box::new(move |time, NoteOn((notenum, velocity))| {
            let time = time - 0.05;
            ctrl1
                .lock()
