...
pub fn sine<A: Node<Output = C1f64>>(frequency: A) -> impl Node<Output = C1f64> {
...
pub fn square<A: Node<Output = C1f64>>(frequency: A, pwm: f64) -> impl Node<Output = C1f64> {
    let acc = Accumulator::new(frequency, 1.0.into());
    Map::new(acc, move |v| C1f64::from(if v.get_m() < pwm { -1.0 } else { 1.0 }))
}

pub fn retriggerable_sine<A: Node<Output = C1f64> + 'static>(
    frequency: A,
) -> (impl Node<Output = C1f64>, impl FnMut(f64)) {
...
pub fn retriggerable_saw<A: Node<Output = C1f64> + 'static>(
    frequency: A,
) -> (impl Node<Output = C1f64>, impl FnMut(f64)) {
    let acc = Controllable::new(EventControlInplace::new(Accumulator::new(
        frequency,
...
pub fn resetable_acc<A: Node<Output = C1f64> + 'static>(
    frequency: A,
) -> (
    Controllable<EventControlInplace<SetValueAtTime<A>, Accumulator<A>>>,
    impl FnMut(f64, f64),
) {
    let acc = Controllable::new(EventControlInplace::new(Accumulator::new(
        frequency,
